...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        # 计算G Norm和G Diver的validation loss（每个epoch都算一次）
        val_g_norm = 0
        val_g_diver = 0
        val_num_g = 0
        with torch.no_grad():
            for val_imgs, val_labels in DataLoader(svhn_test, batch_size=batch_size, shuffle=False, num_workers=1):
                ...
                val_num_g += val_imgs.size(0)
        if epoch == 0:
            g_norm_val_curve = []
            g_diver_val_curve = []
        g_norm_val_curve.append(val_g_norm / val_num_g)
        g_diver_val_curve.append(val_g_diver / val_num_g)
    ...
    targeted_model_file_name = './SVHN_target_modelC_dynamic(PGD)_loss3.pth'
    torch.save(target_model.state_dict(), targeted_model_file_name)
    target_model.eval()

    # 保存最优generator生成的扰动图和原图
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('distillation_loss_curve_dynamic(PGD)_gnorm.png')
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('distillation_loss_curve_dynamic(PGD)_gdiver.png')
    plt.show()
